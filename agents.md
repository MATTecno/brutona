...
- `official.ts` e `official-assets.json` controlam fotos e videos, com origem rastreavel. Nove produtos usam fotos associadas ao produto, tres usam fotos reais contextuais com legenda obrigatoria e cinco usam "Foto em breve". Nenhuma imagem gerada por IA deve voltar a aplicacao. A associacao editorial nao e uma certificacao comercial.
...
- Endereco confirmado pelo responsavel: Rua da Paciencia, 207 (com acentuacao em `brand.ts`), substituindo a divergencia do brief. Os 17 precos e unidades atuais foram aprovados; preservar aviso de alteracao. Quarta-feira: 09:00-20:00.
- `Photo.usage` aceita `product` (padrao), `category` e `serving`; contexto exige `note`, visivel no card e no modal. Kit Caixa usa foto 1, alho-poro usa foto 96 como categoria, Kit Feijoada usa foto 40 como sugestao de preparo. Orelhinha, Pezinho, Papada, Frango e Costelinha ficam sem foto. Nao inferir correspondencias de cortes ou recheios.
- Testes obrigatorios nao leem o acervo ignorado. Fixtures independentes em `tests/fixtures` registram contratos aprovados e hashes dos logos; nao regenerar expectativas a partir do codigo testado. Auditoria opcional: `npm run audit:references`.
- Node 22, `npm ci`, lint, typecheck, testes, build e Chromium/Playwright sao obrigatorios. O Playwright gerencia `next start` na porta 3100; `TEST_BASE_URL` usa servidor externo sem gerencia-lo. Nao iniciar dev server para validar build de producao.
- GitHub e Vercel ja configurados pelo responsavel. Workflow `quality` para PR/push na `master`; protecao exige configuracao e verificacao no GitHub, nao deve ser declarada ativa so pela existencia do YAML. Nao mudar hospedagem nem fazer push/deploy sem solicitacao. Dominio informado: `brutonacharcutaria.com.br`, configuracao pelo responsavel.
