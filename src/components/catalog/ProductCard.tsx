...
export function ProductCard({ product }: { product: Product }) {
  return <Dialog.Root>
    <article className="product-card" data-product-id={product.id}>
      <Dialog.Trigger className="product-trigger" aria-label={`Ver detalhes de ${product.name}`}>
        <div className={`product-photo category-${product.category}`}><ProductPhoto product={product} /><span className="product-category">{categoryLabels[product.category]}</span>{product.photo?.origin === "generated" && <span className="illustrative-badge">Imagem ilustrativa</span>}<span className="product-open" aria-hidden="true"><ArrowUpRight size={22} /></span></div>
    ...
    <Dialog.Portal>
      ...
      <Dialog.Content className="product-modal" aria-describedby={`notice-${product.id}`}>
        ...
        <div className={`modal-photo category-${product.category}`}><ProductPhoto product={product} />{product.photo?.origin === "generated" && <span className="illustrative-badge">Imagem ilustrativa</span>}</div>
        <div className="modal-body"><p className="eyebrow">{categoryLabels[product.category]}</p><Dialog.Title>{product.name}</Dialog.Title>
          ...
          {product.photo?.note && <p className="product-description">{product.photo.note}</p>}
